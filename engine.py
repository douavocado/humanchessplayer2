...
class Engine:
    ...
    def get_human_probabilities(self, board : chess.Board, game_phase: str, log:bool = True):
        ...
        if is_under_mate_threat(board, board.turn, engine=self.stockfish_engine):
            scorer = self.human_scorers["defensive_tactics"]
            if log: 
                self.log += "Using defensive tactics model. \n"
        else:   
            scorer = self.human_scorers[game_phase]
            if log:
                self.log += "Using {} model. \n".format(game_phase)
    ...
    def _alter_move_prob_nn(self, move_dic : dict, board:chess.Board, prev_board:chess.Board = None, prev_prev_board:chess.Board = None, log:bool = True):
        ...
        """
        start = time.time()
        altered_move_dic, log_str = self.move_prob_altering_model.forward_numpy(move_dic, board, prev_board, prev_prev_board)
        if log:
            self.log += log_str
            self.log += "Time taken for move prob alteration: {} seconds \n".format(time.time() - start)
...
###############################################################################
        ...
        if before_king_danger < 250:
            ...
            for move_uci in move_dic.keys():
                ...
                if new_king_danger <= 0:
                    ...
                    protect_king_moves.append(board.san(chess.Move.from_uci(move_uci)))
            if log:
                self.log += "Found moves that protect our vulnerable king: {} \n".format(protect_king_moves)
    ...
    def _set_mood(self):
        ...
        if len(self.input_info["fens"]) >= 3:
            ...
            last_eval = extend_mate_score(self.stockfish_engine.analyse(last_avail_board, limit=chess.engine.Limit(time=0.02))["score"].pov(self.input_info["side"]).score(mate_score=2500))
