...
**Per-game character** (`_sample_game_character`, resampled whenever `update_info` sees the ply count go backwards — the engine instance persists across games; `new_game()` forces it): three draws that vary *between* games what per-move noise can't — `game_pace_sf` (lognormal, scales every think time; applied **after** the phase/mood envelopes because the opening's `**0.2` compression crushes upstream factors), `game_premove_sf` (scales every premove-search probability in `make_move`, including the takeback-only and ponder-dic searches), and `game_snap_gate` (the intuition-gate probability, Uniform(0.45, 0.85), mean 0.65 = the old fixed value). Constants `GAME_*` in `common/constants.py`. Rationale: cheat_detection's Brown–Forsythe variance test flagged the bot as far too consistent game-to-game on every timing feature.

**Hesitation before the mistake** (`_adjust_time_for_move_loss`): after the move is chosen, if it gives up ≥ `MISTAKE_HESITATION_WC_LOSS` win probability (read from the existing full-width analysis — no extra Stockfish work), the already-decided think time is stretched by Uniform(1.3, 2.2) with probability 0.55; skipped in "hurry" mood or under 15s (scramble errors must stay fast). Restores the human-positive per-game correlation between move time and move loss (humans ≈ +0.10; the engine's errors come from the human-probability sampling and are otherwise independent of think time). `MISTAKE_HESITATION_*` in `common/constants.py`.
