...
- `cheat_detection/` — offline human-likeness diagnostic (CLI `analyze.py`, Tkinter `gui.py` with per-feature distribution drill-down; see its README). Analysis depth defaults to 10 (benchmarked ~24× faster than the original 18). Bot analysis and the baseline JSON must use the same depth — mismatched depths skew every engine-derived feature; baselines store per-game feature values (`values`) for the GUI histograms. Baselines/runs/cache are gitignored.
